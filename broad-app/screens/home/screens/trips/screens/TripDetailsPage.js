...
export default function TripDetailsPage({ navigation, route }) {
  ...
  const fetchItems = async function () {
    let response = await fetch(
      `${api_endpoint}${route.params.flatlistIdentifier}/${route.params.pk}/`,
      {
        method: "GET",
        headers: {
    ...
    response.passengers.forEach((passenger, index) => {
      passengerList.push(
        <PassengerCard
          key={index}
          username={passenger.profile_name}
          imageURL={passenger.profile_picture}
        />
      );
    });
    ...
    if (mapRef.current) {
      mapRef.current.fitToCoordinates(
        [response.departure_coordinates, response.destination_coordinates],
        {
    ...
    if ("active_trips".localeCompare(route.params.flatlistIdentifier) != 0)
      return;
      ...
      headerRight: () => (
        <Menu style={styles.menuContainer}>
          ...
          <MenuOptions customStyles={styles.popupMenu}>
            ...
            />
            <MenuOption
              text={response.is_hidden ? "Göster" : "Gizle"}
              onSelect={() => {
                onHideTrip(response.is_hidden);
              }}
            />
  ...
  const onHideTrip = (isHidden) => {
    ...
    Alert.alert(
      ...
      tripDetails.isHidden
        ? "Yolculuğunuz gösterilecektir."
        : "Yolculuğunuz gizelenecektir.",
      [
        {
          text: "Vazgeç",
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            const response = await fetch(
              `${api_endpoint}trips/hide/${route.params.pk}`,
              {
                method: "PATCH",
                headers: {
    ...
    Alert.alert("Emin misiniz?", "Yolculuğunuz yayından kaldırılacaktır.", [
      ...
      },
      {
        ...
        onPress: async () => {
          await renewCSRFToken();
          const response = await fetch(
            `${api_endpoint}trips/delete/${route.params.pk}`,
            {
              method: "DELETE",
              headers: {
                "X-CSRFToken": csrftoken,
        ...
        <View style={styles.foregroundContainer}>
          <ScrollView style={{ width: "100%" }}>
            <View style={styles.driverHeaderContainer}>
              <TouchableOpacity style={styles.driverProfileContainer}>
                ...
                <View style={styles.driverStarsContainer}>{starsList}</View>
              </TouchableOpacity>
              <ImageBackground
                source={require("../../../../../images/text-bubble.png")}
                resizeMode="stretch"
              >
            ...
            <View style={{ margin: 10 }}>
              ...
              <View style={styles.statsContainer}>
                <View style={styles.statsElementContainer}>
                  ...
                  <Text>
                    {tripDetails.emptySeats}/{tripDetails.maxSeats}
                  </Text>
                </View>
                <View style={styles.statsElementContainer}>
                  <FontAwesome size={20} color={colors.deepblue} name="car" />
                  <Text>
                    {tripDetails.carModel
                      ? tripDetails.carModel
                      : "Model Belirtilmemiş"}
                  </Text>
                </View>
            ...
            {route.params.onGoing && (
              <>
                ...
                <View style={{ padding: 10 }}>
                  ...
                  >
                    <Text style={styles.buttonText}>Yolculuğu Bitir</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
          ...
          {route.params.onGoing && (
            <>
              <Animated.View
                style={[
                  styles.hoveringButtonsContainer,
                  { transform: [{ translateY: emergencyButtonLocation }] },
                ]}
              >
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    Animated.timing(emergencyButtonLocation, {
                      toValue: emergencyMenuOpen ? 0 : -100,
                      duration: 500,
                      useNativeDriver: true,
                    }).start();
                    Animated.timing(expandButtonOpacity, {
                      toValue: emergencyMenuOpen ? 0 : 1,
                      duration: 500,
                      useNativeDriver: true,
                    }).start();
                  }}
              ...
              <Animated.View
                style={[
                  styles.expandedEmergencyButtonsContainer,
                  { opacity: expandButtonOpacity },
                ]}
              >
                <TouchableOpacity style={styles.expandedEmergencyButtons}>
                  <Text style={styles.expandedEmergencyText}>
                    Bizimle irtibata gec
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.expandedEmergencyButtons}>
                  <Text style={styles.expandedEmergencyText}>112'yi ara</Text>
                </TouchableOpacity>
              </Animated.View>
            </>
          )}
